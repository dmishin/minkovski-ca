...
var $, M, animateRotations, canvas, ctx, drawAllBranches, getAjax, linspace, pad0, savedContext, uploadToServer;
...
savedContext = function(ctx, func) {
  var rval;
  ctx.save();
  try {
    rval = func();
...
animateRotations = function() {
  var a, arrowH, arrowW, drawAxes, drawConics, drawFrame, drawHyperbola, drawPath, height, margin, nframes, paths, runUpload, size;
  nframes = 100;
  canvas.width = 480;
  canvas.height = 240;
  paths = [
    ...
    ref,
    results;
        ref = linspace(0, 2 * Math.PI, 100);
        results = [];
        for (j = 0, len = ref.length; j < len; j++) {
          a = ref[j];
          results.push([Math.cos(a) * 20,
    Math.sin(a) * 20]);
        }
        return results;
      })(),
    ...
    ref,
    results;
        ref = linspace(-Math.log(70 / 20), Math.log(70 / 20), 100);
        results = [];
        for (j = 0, len = ref.length; j < len; j++) {
          a = ref[j];
          results.push([Math.cosh(a) * 20,
    Math.sinh(a) * 20]);
        }
        return results;
      })(),
  ...
  drawPath = function(path, tfm) {
    var i, j, len, ref, ref1, ref2, x, xy, y;
    ctx.strokeStyle = (ref = path.color) != null ? ref : "#000";
    ctx.lineWidth = (ref1 = path.width) != null ? ref1 : 1;
    ctx.beginPath();
    ref2 = path.points;
  ...
  drawConics = function(A, radii) {
    return savedContext(ctx, function() {
      var j, len, radius, results;
      ctx.lineWidth = 1;
      ctx.setLineDash([5, 5]);
      results = [];
      for (j = 0, len = radii.length; j < len; j++) {
        radius = radii[j];
        ctx.strokeStyle = "#faa";
        drawHyperbola(M.smul(1.0 / (Math.pow(radius, 2)), A));
        ctx.strokeStyle = "#aaf";
        results.push(drawHyperbola(M.smul(-1.0 / (Math.pow(radius, 2)), A)));
      }
      return results;
    });
  };
  drawHyperbola = function(A) {
    var j, k, len, len1, part, ref, results, x, y;
    ref = drawAllBranches(A, -0.5 * size, -0.5 * height, 0.5 * size, 0.5 * height, 0.1);
    results = [];
    for (j = 0, len = ref.length; j < len; j++) {
      part = ref[j];
      ctx.beginPath();
  ...
  drawFrame = function(angle) {
    ctx.fillStyle = "#fff";
    ctx.fillRect(0, 0, canvas.width, canvas.height);
    return savedContext(ctx, function() {
      var j, k, len, len1, path, radii, tfm;
      //###########################
      ...
      radii = [
        70,
        20 //(r for r in [0 .. size*0.75] by 20)      
      ];
  ...
  runUpload = function(step) {
    ...
    drawFrame(angle);
    return uploadToServer(`animate-rotation-${pad0(4, step)}.png`, function(ajax) {
      if (ajax.status !== 200) {
        console.log("Error");
        return console.log(ajax);
      } else {
        return runUpload(step + 1);
      }
    });
  };
  return runUpload(0);
...
$("#btn-run-rotations").on('click', function() {
  return animateRotations();
});


},{"./matrix2.coffee":3,"./ode_curve_drawing.coffee":4,"jquery":1}],3:[function(require,module,exports){
"use strict";
